...
func (s *loanService) Apply(req models.CreateLoanRequest, userID uuid.UUID) (interface{}, error) {
	...
	if err != nil {
		// TODO: Vulnerability Injection Point — OWASP API6 (Business Flow Abuse)
		// A06 Vulnerable: skip the nasabah-profile gate — any authenticated user can
		// submit a loan application without completing the required onboarding step.
		...
		s.log.Warn("[VULNERABLE] Loan applied without nasabah profile — A06 bypass",
			zap.String("user_id", userID.String()),
		)
		nasabah = &models.Nasabah{
			ID:          uuid.New(),
			UserID:      userID,
			FullName:    "Auto-generated (A06 bypass)",
			NIK:         fmt.Sprintf("%016d", time.Now().UnixNano()%10_000_000_000_000_000),
			Phone:       "000000000000",
			Address:     "Auto-generated by vulnerable mode",
			DateOfBirth: time.Date(1990, 1, 1, 0, 0, 0, 0, time.UTC),
		}
		if createErr := s.nasabahRepo.Create(nasabah); createErr != nil {
			return nil, errors.New("nasabah profile required before applying for a loan")
		}
	}
